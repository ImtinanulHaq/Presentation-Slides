...
print("="*80)
print(f"\nInput: Bitcoin research paper ({len(bitcoin_content.split())} words)")
print("Expected behavior:")
print("  ✓ Auto-enable chunking (>300 words)")
print("  ✓ Generate slides from multiple chunks")
print("  ✓ Each slide has 4-8 bullet points")
...
try:
    ...
    print(f"\nValidating {len(slides)} slides for 4-8 bullet points...\n")
    ...
    for i, slide in enumerate(slides, 1):
        ...
        status = "✓" if 4 <= bullet_count <= 8 else "✗"
        print(f"{status} Slide {i}: '{title[:50]}...' - {bullet_count} bullets")
        
        if not (4 <= bullet_count <= 8):
            violations.append({
                'slide': i,
                'title': title,
                'count': bullet_count,
    ...
    print(f"\n{'='*80}")
    if not violations:
        print("✓ SUCCESS: All slides have 4-8 bullet points!")
    else:
        print(f"✗ VIOLATIONS FOUND: {len(violations)} slides don't meet requirements")
        for v in violations:
            print(f"\n  Slide {v['slide']}: {v['title']}")
            print(f"  Expected: 4-8 bullets, Got: {v['count']}")
            print(f"  Bullets: {v['bullets'][:3]}...")
